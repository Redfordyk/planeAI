  ...
  const handleUpdatePublishSettings = async (payload: Partial<TProjectPublishSettings>) => {
    ...
    await updatePublishSettings(workspaceSlug.toString(), projectId, payload.id, payload).then((res) => {
      setToast({
        ...
        title: t("toast.success.title", { defaultValue: "Success!" }),
        message: t("publish_modal.toast.settings_updated"),
      });
    ...
    await unPublishProject(workspaceSlug.toString(), projectId, publishId)
      .catch(() =>
        setToast({
          ...
          title: t("toast.error"),
          message: t("publish_modal.toast.unpublish_failed"),
        })
      )
  ...
  const handleFormSubmit = async (formData: Partial<TProjectPublishSettings>) => {
    if (!selectedLayouts || selectedLayouts.length === 0) {
      setToast({
        type: TOAST_TYPE.ERROR,
        title: t("toast.error"),
        message: t("publish_modal.toast.select_layout"),
      });
      return;
    }
  ...
  const handleCopyLink = () =>
    copyTextToClipboard(publishLink).then(() =>
      setToast({
        type: TOAST_TYPE.SUCCESS,
        title: "",
        message: t("publish_modal.toast.link_copied"),
      })
    );
      ...
      <form onSubmit={handleSubmit(handleFormSubmit)}>
        <div className="flex items-center justify-between gap-2 p-5">
          ...
          {isProjectPublished && (
            ...
            >
              {isUnPublishing ? t("publish_modal.unpublishing") : t("publish_modal.unpublish")}
            </Button>
          )}
        </div>
        ...
        {fetchSettingsLoader ? (
          ...
          <div className="space-y-4 px-5">
            ...
            <div className="space-y-4">
              ...
              <div className="relative flex items-center justify-between gap-2">
                <div className="text-13">{t("publish_modal.allow_comments")}</div>
                <Controller
                  control={control}
              ...
              <div className="relative flex items-center justify-between gap-2">
                <div className="text-13">{t("publish_modal.allow_reactions")}</div>
                <Controller
                  control={control}
              ...
              <div className="relative flex items-center justify-between gap-2">
                <div className="text-13">{t("publish_modal.allow_voting")}</div>
                <Controller
                  control={control}
        ...
        <div className="relative mt-4 flex items-center justify-between border-t border-subtle px-5 py-4">
          <div className="flex items-center gap-1 text-13 text-placeholder">
            ...
            <div className="text-13">{t("publish_modal.anyone_with_link")}</div>
          </div>
          {!fetchSettingsLoader && (
              ...
              {isProjectPublished ? (
                isDirty && (
                  <Button variant="primary" size="lg" type="submit" loading={isSubmitting}>
                    {isSubmitting ? t("publish_modal.updating") : t("publish_modal.update_settings")}
                  </Button>
                )
              ) : (
                <Button variant="primary" size="lg" type="submit" loading={isSubmitting}>
                  {isSubmitting ? t("publish_modal.publishing") : t("publish")}
                </Button>
              )}
            </div>
          )}
